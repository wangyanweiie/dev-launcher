...
| 方式 | 说明 |
|------|------|
| **双击启动（推荐）** | macOS：`scripts/macos/启动 Dev Launcher.command`；Windows：`scripts/windows/启动 Dev Launcher.bat` |
| 命令行 | macOS/Linux：`bash scripts/macos/start-dev-launcher.sh`；Windows：`scripts\windows\start-dev-launcher.bat` |
| 通用 | `pnpm start`（需已配置 `config.json`） |
...
2. macOS 若无法双击：终端执行 `chmod +x scripts/macos/*.sh scripts/macos/*.command`
3. Windows 若 PowerShell 提示无法运行脚本：直接用 `.bat` 启动（已内置 `-ExecutionPolicy Bypass`）
4. 脚本会自动生成 `config.json`；按提示填写 `scanRoot`（如 macOS `~/Company`、Windows `C:\Users\你\Company`）
...
| 字段 | 说明 | 默认 |
|------|------|------|
| `taskProfiles` | 命名配置档（合并 `taskEnv` / `nodeOptions`），启动时自动套用 | 见 example |
| `defaultTaskProfile` | 启动 dev 时使用的配置档名 | `"balanced"` |
| `maxRunningTasks` | 同时运行的 dev 上限，`0` 不限制 | `0` |
| `idleAutoStopMinutes` | 无日志超过该分钟数自动停止，`0` 关闭 | `0` |
| `forceColor` | 子进程是否设置 `FORCE_COLOR=1` | `true` |
| `logSubscribeOnly` | WebSocket 仅向已订阅任务推送 log（需前端 subscribe） | `false` |
...
| POST | `/api/tasks/start` | 启动任务（环境由 `defaultTaskProfile` / `taskProfiles` 决定） |
